...
def load_local_stopwords(path: str = 'stopwords_de.txt') -> Tuple[Set[str], List[str]]:
    """Lade lokale Stopwort-Liste.

    Gibt konsistent ein Tuple (set_single, list_phrases) zurück.
    Bei Fehlern werden leere Strukturen zurückgegeben.
    """
    try:
        import unicodedata
...
def prepare_stopwords_and_lemmatizer():
    ...
    try:
        ...
        if not stop_words and not stop_phrases:
            try:
